...
class Vision:
    ...
    def __init__(self, needle_img_path, method=cv.TM_CCOEFF_NORMED):
        ...
        if needle_img.shape[2] == 4:  # If the image has an alpha channel, drop it
            needle_img = cv.cvtColor(needle_img, cv.COLOR_BGRA2GRAY)
        else:  # Otherwise, convert to grayscale directly
    ...
    def find(self, skull_img, threshold=0.5, mode=None):       
        # Ensure the skull_img is in grayscale
        if len(skull_img.shape) == 3:
            skull_img = cv.cvtColor(skull_img, cv.COLOR_BGR2GRAY)

        # Convert to CV_8U if needed
        if skull_img.dtype != np.uint8:
            skull_img = skull_img.astype(np.uint8)
        if self.needle_img.dtype != np.uint8:
            self.needle_img = self.needle_img.astype(np.uint8)

        # Run the OpenCV algorithm
        result = cv.matchTemplate(skull_img, self.needle_img, self.method)
